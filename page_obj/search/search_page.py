...
class SearchPage(SearchProperties):
    ...
    def search_item(self, food, expected=None):
        ...
        logging.info("Waiting for SEARCH_BOX and clearing it")
        searchbox = self.wait.until(
            EC.element_to_be_clickable(SearchLocators.SEARCH_BOX)
        )
        searchbox.clear()
        ...
        self.wait.until(
            EC.visibility_of_element_located(
                (By.XPATH, "//div[@class='main-auto-suggestion']")
            )
        )
        ...
        if expected:
            ...
            first_item = self.wait.until(
                EC.element_to_be_clickable(
                    (By.XPATH, "(//div[@class='main-auto-suggestion']//span)[1]")
                )
            )
            first_item.click()
